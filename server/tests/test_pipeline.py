...
def proj(**kwargs: object) -> PipelineProjection:
    ...
    transfer_mode = kwargs.pop("transfer_mode", "running")
    return project(
        asset,
        assignment,
        client,
...
def test_row4_asset_failed_no_detail() -> None:
    assert (
        proj(asset=make_asset(status="failed", status_detail=None)).detail
        == "transcode failed: unknown error"
    )
...
def test_transfer_mode_paused_queues_ready_assignment() -> None:
    result = proj(transfer_mode="paused")
    assert result.status == "queued"
    assert result.substate == "paused"
    assert result.detail == "transfers paused"


def test_transfer_mode_stopped_queues_ready_assignment() -> None:
    result = proj(transfer_mode="stopped")
    assert result.status == "queued"
    assert result.substate == "paused"
    assert result.detail == "transfers stopped"


def test_transfer_mode_running_preserves_normal_flow() -> None:
    result = proj(transfer_mode="running")
    assert result.status == "queued"
    assert result.substate == "waiting_for_agent"
    assert result.detail == "waiting for agent to pick up"
...
def test_row7_offline_threshold_not_stalled_at_boundary() -> None:
    ...
    assert (
        proj(client=make_client(last_seen=NOW - timedelta(seconds=900))).substate != "agent_offline"
    )


def test_row7_offline_one_second_over_threshold() -> None:
    assert (
        proj(client=make_client(last_seen=NOW - timedelta(seconds=901))).substate == "agent_offline"
    )
...
def test_row11_stalled() -> None:
    ...
    result = proj(
        assignment=make_assignment(bytes_downloaded=500_000, bytes_downloaded_updated_at=stale)
    )
    assert result.status == "transferring"
...
def test_row12_rate_one_sample_is_none() -> None:
    ...
    assert (
        proj(
            assignment=make_assignment(bytes_downloaded=250_000), rate_samples=samples
        ).transfer_rate_bps
        is None
    )
...
def test_row12_rate_two_samples_under_1s_is_none() -> None:
    ...
    assert (
        proj(
            assignment=make_assignment(bytes_downloaded=300_000), rate_samples=samples
        ).transfer_rate_bps
        is None
    )
